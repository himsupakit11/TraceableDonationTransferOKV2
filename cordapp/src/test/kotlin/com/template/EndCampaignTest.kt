...
//        listOf(Fundraiser, Bank, Donor, Recipient).forEach { it.registerInitiatedFlow(AutoOfferFlow.RecordTransactionAsObserver::class.java) }
    ...
    private val OneSecondsFromNow: Instant get() = calculatedDeadlineInSeconds(7L)
...
//        val temp = Donor.startFlow(flow1).getOrThrow()
        ...
        val makeDonationFlow2= MakeDonation.Initiator(2000.DOLLARS, campaignId,"Credit Card")
        logger.info("MakeDonationFlow: $makeDonationFlow")
...
//        logger.info(acceptDonationTransaction3.tx.toString())
        ...
        /*********************/
        /**Make receipt flow*/
        val makeReceiptFlow2 = MakeReceipt.Initiator(campaignId, 2000.DOLLARS)
        val startReceiptFlow2 = Fundraiser.startFlow(makeReceiptFlow2).getOrThrow()
        logger.info("Start ReceiptFlow Successfully  ")
        logger.info(makeReceiptFlow2.toString())
        logger.info(startReceiptFlow2.tx.toString())
...
//        logger.info("===========================================")
        ...
        val x = Bank.services.vaultService.queryBy<Receipt>().states
        val j = Bank.services.vaultService.queryBy<Donation>().states
        val y = Bank.services.vaultService.queryBy<Campaign>().states
//        val z = Fundraiser.services.vaultService.queryBy<Receipt>().states
        logger.info("===========================================")
        logger.info("Bank Receipt Vault : $x")
        logger.info("Bank Donation Vault     : $j")
        logger.info("Bank Campaign Vault      : $y")
//        logger.info("Fundraiser Receipt Vault: $z")
        logger.info("===========================================")
